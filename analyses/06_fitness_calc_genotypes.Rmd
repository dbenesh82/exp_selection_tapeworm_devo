...
```{r}
# # worms that developed much faster than expected - how much does the residual correlation affect infectivity compared to the genetic correlation
# # prob of infecting fish 12 dpi
# cg_finf14r <- cerc_finf$Sol[,"traitschisto_inf"] 
# # effect of fast genotype
# fg_finf14r <- qnorm(0.9)*
#   (cerc_finf$VCV[,"traitcerc_bi:traitschisto_inf.units"])/sqrt(cerc_finf$VCV[,"traitcerc_bi:traitcerc_bi.units"])
# # effect of slow genotype
# sg_finf14r <- qnorm(0.1)*
#   (cerc_finf$VCV[,"traitcerc_bi:traitschisto_inf.units"])/sqrt(cerc_finf$VCV[,"traitcerc_bi:traitcerc_bi.units"])
# summary(pnorm(cg_finf14r+fg_finf14r))
# summary(pnorm(cg_finf14r))
# summary(pnorm(cg_finf14r+sg_finf14r))
```
```{r}
# summary(pnorm(cg_finf14+fg_finf14) - pnorm(cg_finf14))
```
```{r}
# summary(pnorm(cg_finf14r+fg_finf14r) - pnorm(cg_finf14r))
```
...
```
Note that we did not fit a model with a genotype by environment interaction (i.e. the model did not test whether genotypes differ more at 12 dpi than 16 dpi). Let's fit that model.

```{r}
# thin_finf <- 75
# burnin_finf <- 1000
# nit_finf <- thin_finf * 500 + burnin_finf
# 
# m2_finf_fam_GxE <- 
#   MCMCglmm(schisto_inf ~ ib + fish_initial_bm_cen + fish_sex_cen + fish_exp_on_cop_dpe_fct,  
#            random = ~ cop_block + idh(fish_exp_on_cop_dpe_fct):worm_full_sib_fam_id,
#            family = "threshold",
#            data = dat_finf,
#            prior = 
#              list(R = list(V = 1, fix = 1),
#                   G = list(G1 = list(V = 1, nu = 1000, alpha.mu = 0, alpha.V = 1),
#                            G2 = list(V = diag(3), nu = 3, alpha.mu = c(0,0,0), alpha.V = diag(3)))),
#            verbose=F,
#            nitt = nit_finf, 
#            thin = thin_finf,
#            burnin = burnin_finf
...
Here it is. The variance among families decreases from 12 to 16 dpi, which fits the differences we saw among lines in the final generation. However, the confidence intervals are wide, which makes sense, given that relatively few fish were exposed per family per time point.

```{r}
# summary(m2_finf_fam_GxE)
```

We can clearly see that when looking at the posterior trace.

```{r}
# plot(mcmc.list(
#   m2_finf_fam_GxE$VCV[,"fish_exp_on_cop_dpe_fct12.worm_full_sib_fam_id"],
#   m2_finf_fam_GxE$VCV[,"fish_exp_on_cop_dpe_fct14.worm_full_sib_fam_id"],
#   m2_finf_fam_GxE$VCV[,"fish_exp_on_cop_dpe_fct16.worm_full_sib_fam_id"]
#   ), density = F
#   )
```

So, estimating different genetic co(variances) at each infection time point is probably beyond the capability of the data, especially since differences between families were small.
...
fitness_fish <- left_join(fitness_fish, pred_plerx, by = "iter")%>%
  ...
  mutate(eggs_scl = case_when(
           ...
           grp == "fast" ~ 
             exp(egg_int + log_ww_cen * (log(pler_size) - log(mean(dat_bred$worm_bm_mg, na.rm = T))) +
                   qnorm(0.9)*cov_cerc_fec/sqrt(var_cerc_fec)),
           grp == "slow" ~ 
             exp(egg_int + log_ww_cen * (log(pler_size) - log(mean(dat_bred$worm_bm_mg, na.rm = T))) +
...
ggsave(f_sz, filename = "../figs/figSZ_fitness_grps.png", width = 75*3.5, height = 75*2, units = "mm")
```
